...
def stim_fun_dummy(stim, duty_cyle_on=1, freq=10,t = 0):
    '''
    A simple stimulation function, 
    will run for a set on time (duty_cycle_on) in seconds
    with a frequency in Hz and with a stim profile set by 'stim'
    ...
    stimout = []
    trest = 1/freq
    oncyc_end = duty_cyle_on
    while t < oncyc_end:
        stimsig = stim.run()
        stimout.extend(stimsig)
        t = t+len(stimsig) + trest
    return stimout,t

    
class DS8R_dummy:
    """
    A dummy version of the Python controller for DS8R device. 
    To allow testing configurations without requiring a connection to the DS8R.
    ...
    def run(self, force=False):
        ...
        """
        if self.demand <= 500 or force:
            # here we will want to create a stim output that matches the parameters given, that can then be plotted.
            if self.polarity==1:
                stimsig = [self.demand]*self.pulse_width
            elif self.polarity==2:
                stimsig = [-self.demand]*self.pulse_width
            else: # polarity ==3
                stimsig = [self.prevpol*self.demand]*self.pulse_width
                self.prevpol = self.prevpol*-1 # flip the sign
            # add the dwell time
            stimsig.extend([0]*self.dwell)
            # add recovery if biphasic
            if self.mode == 2: 
                stimsig.extend([-self.demand*(self.recovery/100)]*(self.pulse_width*(100/self.recovery)))
            else:
                stimsig.extend([0]*self.recovery)
        else:
            raise ValueError(
